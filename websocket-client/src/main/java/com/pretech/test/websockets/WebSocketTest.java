...
public class WebSocketTest {
	...
	public void onMessage(String message, Session session) throws IOException,
		...
		if (message.contains("sendsmall")) {
			sendMsgs(session, 10,100);
		} else if (message.contains("sendsmallclose")) {
			sendMsgs(session, 10,100);
			session.close(new CloseReason(CloseCodes.CLOSED_ABNORMALLY, "abnormal close"));
		} else if (message.contains("errorconnect")) {
			session.close();
		} else if (message.contains("sendlargewaitsendlarge")) {
			sendMsgs(session, 1000,100);
			Thread.sleep(5000);
			sendMsgs(session, 1000,100);
		} else if (message.contains("sendlarge")) {
			sendMsgs(session, 1000,100);
		} else if (message.contains("sendsmallerror")) {
			sendMsgs(session, 10,100);
			session.close(new CloseReason(CloseCodes.CLOSED_ABNORMALLY, "abnormal close"));
		} else if (message.contains("sendlargedelaysendlargenodelay")) {
			sendMsgs(session, 1000,3000);
			Thread.sleep(5000);
			sendMsgs(session, 1000,100);
		}
